...
interface Props {
  ...
  batchQueuedIds?: Set<string>;
  onToggleBatchQueue?: (company: Company) => void;
}

export function AdminCompanyTable({ companies, queuedIds, onToggleQueue, batchQueuedIds, onToggleBatchQueue }: Props) {
    ...
    <div className="card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm" style={{ minWidth: '900px' }}>
          <thead>
            <tr style={{ backgroundColor: '#EAF5F5', borderBottom: '2px solid #3A9E9E' }}>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-nowrap">
                      ...
                      {onToggleBatchQueue && (
                        <button
                          onClick={() => onToggleBatchQueue(company)}
                          className="action-btn"
                        >
                          {batchQueuedIds?.has(company.id) ? '✓ Batched' : 'Batch'}
                        </button>
                      )}
                      <Link href={`/mb-hub/companies/${company.id}`} className="action-btn">View</Link>
                      <Link href={`/mb-hub/companies/${company.id}/edit`} className="action-btn">Edit</Link>
                      <button onClick={() => handleDelete(company)} className="action-btn action-btn-danger">Delete</button>
                    </div>
                  </td>
                </tr>
              );
