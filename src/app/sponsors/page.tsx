...
const TIER_CONFIG = [
  { key: 'sponsors_community_description' as const, name: 'Community', price: '$500', accent: '#3A9E9E' },
  { key: 'sponsors_growth_description' as const, name: 'Growth', price: '$1,000', accent: '#F26522' },
  { key: 'sponsors_premier_description' as const, name: 'Premier', price: '$2,500', accent: '#1B3A52' },
];
...
export default async function SponsorsPage() {
  ...
  const introParagraphs = content.sponsors_intro.split('\n\n').filter(Boolean);
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F8F9FA' }}>
      ...
      <main className="flex-1 max-w-2xl md:max-w-3xl lg:max-w-5xl mx-auto w-full px-6 pt-12 pb-24">
        <h1 className="text-4xl font-extrabold mb-2" style={{ color: '#1B3A52' }}>
          {content.sponsors_headline}
        </h1>
        <div className="w-10 h-1 rounded-full mb-6" style={{ backgroundColor: '#3A9E9E' }} />

        {introParagraphs.map((p, i) => (
          <p key={i} className="text-gray-600 text-lg leading-relaxed mb-4 max-w-2xl">
            {p}
          </p>
        ))}

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10 mb-16">
          {TIER_CONFIG.map(({ key, name, price, accent }) => {
            const perks = content[key].split('\n').filter(Boolean);
            return (
              <div key={name} className="bg-white rounded-2xl border border-gray-200 p-8 flex flex-col">
                <div className="mb-6">
                  <div className="w-8 h-1 rounded-full mb-4" style={{ backgroundColor: accent }} />
                  <h2 className="text-xl font-bold mb-1" style={{ color: '#1B3A52' }}>{name}</h2>
        ...
        <div className="pt-10 border-t border-gray-200">
          ...
          <a href="mailto:will@startupmb.com" className="btn-primary">
