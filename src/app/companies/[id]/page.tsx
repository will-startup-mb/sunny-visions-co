...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const [company] = await db
    .select()
    .from(companies)
    .where(and(eq(companies.id, id), eq(companies.is_published, true)));
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F4F8FB' }}>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
        <Link href="/" className="text-sm mb-6 inline-flex items-center gap-1" style={{ color: '#3A9E9E' }}>
          ← Back to directory
        </Link>

        <div className="card p-6 sm:p-8 mt-3">
          {/* Company header */}
        ...
        <div className="mt-8 flex flex-col items-center gap-4">
          <Link href="/" className="btn-secondary">← Back to all companies</Link>
